...
void Graph::update_edges(std::string series_code, double threshold, std::string relation) {
    bool edges_added = false; // Tracking if a min of one edge was added
    for(std::unordered_map<std::string, CountryTotal>::iterator i = node_map.begin(); i!= node_map.end(); i++) {
        for(std::unordered_map<std::string, CountryTotal>::iterator j = node_map.begin(); j!=node_map.end(); j++) {
            ...
            // Getting the two country objects to compare
            CountryTotal& country_1 = i->second;
            CountryTotal& country_2 = j->second;

            // Getting the time series of the respective countires
            ...
            // Getting the mean of the respective time series
            ...
            // Checking if both conditios are met!
            bool condition_one = false;
            bool condition_two = false;

            // Checking if the condition's are met based on user input
            if(relation=="greater") {
                if(mean_1 > threshold) {
                    condition_one = true;
                }
            ...
            // If both conditions are met, add edge!
            if(condition_one && condition_two) {
                ...
                // Add if reltionships hasn't already been added!!!!
                if(relationship[series_code_1][series_code_2].count(relation_tuple) == 0 && relationship[series_code_2][series_code_1].count(relation_tuple) == 0) {
                    // Undirected graph, therefore add edge to both nodes
                    ...
                    edges_added = true; // If at least one edge was added, update condition
                }
            }
        }

    }
    if(edges_added) { // Print success if at least one edge was added!
        std::cout << "success" << std::endl;
    } else{
...
// Function to print out all countires adjacent (connected by one edge) to a user inputted country code
void Graph::adjacent(std::string country_code) {
    ...
    // If country is not in the graph, print out "failure"
    if(i == node_map.end()) {
        std::cout << "failure" << std::endl;
        return;
    }
    std::unordered_map<std::string,std::set<std::string>>::iterator adj_list_i = adj_list.find(country_code);
    // If country is in the graph but has NO adjacent connections, print out "none"
    if(adj_list_i == adj_list.end() || adj_list_i->second.empty()) {
        std::cout << "none" <<std::endl;
        return;
    }
    // iterate over all adjacent country codes
    std::set<std::string> adj_set_i = adj_list_i->second;
    for(std::set<std::string>::iterator j = adj_set_i.begin(); j != adj_set_i.end(); j++) {
        // Find the assoisted country object for the adjacent country code
        std::unordered_map<std::string, CountryTotal>::iterator adj = node_map.find(*j);
        if(adj != node_map.end()) {
            std::cout << adj->second.get_country_name() << " "; // Print name of said adjacent country
        }
    }
    std::cout<<std::endl;
}

// Function to check if a path exists between two countries in the graph using BFS
void Graph::path(std::string country_code_1, std::string country_code_2) {
    // Keep track of visited nodes
    std::set<std::string> visited;
    
    // queue used for classic bfs traversal
    std::queue<std::string> queue;
    queue.push(country_code_1);
    visited.insert(country_code_1);

    // keep going until all nodes have been visited that are reachable
    while(!queue.empty()) {
        std::string current_node = queue.front(); // Get cur node
        queue.pop();
        if(current_node == country_code_2) { // Check if cur node is target country
            std::cout << "true" << std::endl;
            return;
        }
        // Check all adjacent nodes to the current node
        if(adj_list.find(current_node) != adj_list.end()) {
            for(std::set<std::string>::iterator i = adj_list.find(current_node)->second.begin(); i!=adj_list.find(current_node)->second.end(); i++ ) {
                if(visited.find(*i) == visited.end()) { // If adj node hasn't been visited, add to queue and visited
                    queue.push(*i);
                    visited.insert(*i);
                }
...
// Function that prints out relationships (tuple) between two countries in the graph
void Graph::relationships(std::string country_code_1, std::string country_code_2) {
    if(relationship.find(country_code_1) != relationship.end() && relationship.at(country_code_1).find(country_code_2) != relationship.at(country_code_1).end() ) { // Check if there are any reltionship tuples between the two countries
